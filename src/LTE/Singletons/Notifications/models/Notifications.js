    ...
    mutations: {
        ...
        switchNotifications: (state) => {
            if (state.disableNotifications === false) {
                state.sidebarData.tabList.data.find(i => i.name === 'volumes').iconName = 'icon-volume-off';
            } else {
                state.disableTime = 0;
                state.sidebarData.tabList.data.find(i => i.name === 'volumes').iconName = 'icon-volume';
            }
