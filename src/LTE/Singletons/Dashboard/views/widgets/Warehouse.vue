<template>
  <div  :class="[hideWidget === 'warehouse'
  ? (index + 1) % 2 === 0
  ? 'hide-right-widget'
  : 'hide-left-widget' : '',
  'container-dashboard-warehouse']">
    ...
    <div class="warehouse-body">
      <div class="body-coast">
        ...
        <span class="coast-sum">{{ data.widget.cost }}₴</span>
      </div>
      <div class="body-coast">
        <span class="coast-total">Количество свободных ресурсов</span>
        <span class="coast-sum">{{ data.widget.available }}</span>
      </div>
    </div>
    <div class="warehouse-footer">
      <span>Запросы на ресурс</span>
      <span class="footer-num">{{ data.widget.requests }}</span>
    </div>
  </div>
</template>
