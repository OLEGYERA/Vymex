<template>
  <div class="navigation-sidebar-add-observer-ui" v-if="status">
    <sidebar-right class="sidebar-add-observer-plate" @onClose="closeAddSidebar()">
      ...
      <template #main-content>
        <div class="sidebar-info-group">
          <div class="sidebar-header-group">
            <title-caps class="sidebar-add-observer-subtitle">Пользователи</title-caps>
            <input-search :placeholder="'Поиск'"/>
          </div>
          <sidebar-structure-ui :structure="structure"
                                multiply
...
<script>
  ...
  export default {
    name: 'Providers.Navigation.Sidebar.AddObserver.Ui',
    components:{
      TitleCaps,
      InputSearch,
      UnitCheckboxUi,
    ...
    methods:{
      ...mapMutations({
        closeAddSidebar: 'Tasks/closeAddSidebar',
      }),
      // onClick(key){
...
.navigation-sidebar-add-observer-ui{
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  ...
  z-index: 4;
  .sidebar-add-observer-plate::v-deep{
    display: flex;
    height: 100%;
