...
let ANIMATION_1 = [{step:1.5, speed: 1.4, boundary:120}, {step:3, speed: 1.8, boundary:100}]

function anim(el, step){
    let count = 0;
    let anim_steping=el.step
    if(OPTI_ITER) OPTI_ITER = clearInterval(OPTI_ITER)
    OPTI_ITER = setInterval(function () {
      count++;
      anim_steping *= el.speed

        // =count * (el.step * el.speed);
      let delta = (step===1 ? 30 : 0) + (count+anim_steping);
      BIND_NODE.data.on.someEvent({axisY: true, delta: delta});
      if(delta > el.boundary) {
        OPTI_ITER = clearInterval(OPTI_ITER);
        if(step === 0) anim(ANIMATION_1[1], 1)
        return false;
      }
    }, 4);
}
